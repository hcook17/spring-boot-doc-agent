...
| Dependency reproducibility | **Weak — now empirically demonstrated, not just theorized** | No `requirements.txt`/`pyproject.toml` exists; `ast-grep`, `sqllineage`, `pathspec` are all unpinned (`CONSTRAINTS.md` "Runtime prerequisites" item 4). Neither new script adds a third-party dependency (both are stdlib-only, confirmed by their own imports). **2026-07-24 finding**: a fresh environment without `ast-grep`/`sqllineage` installed failed `test_capacity_preflight.py` outright and 7/32 tests in `test_spring_signal_scan.py` — a live reproduction of this exact gap, not a defect introduced by this PR (`spring_signal_scan.py` isn't touched by it). Installing both brought every suite to a clean pass. This reclassifies the adoption gate checklist's dependency-pinning item from aspirational to blocking. |
...
- [ ] Dependencies pinned (`requirements.txt` or `pyproject.toml` for `ast-grep-cli`, `sqllineage`, `pathspec`) — **blocking, not aspirational**: a fresh environment without these installed doesn't just risk future drift, it fails this repo's own test suite today (`CONSTRAINTS.md` item 4's 2026-07-24 finding).
