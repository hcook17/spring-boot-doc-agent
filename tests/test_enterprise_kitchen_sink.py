#!/usr/bin/env python3
"""
test_enterprise_kitchen_sink.py — the whole pipeline, run as real commands
...
followed from that:
  ...
  1. Nothing ran the pipeline's documented command sequence as real
     subprocesses. test_capacity_preflight.py chains four stages in-process
     and stops before the gates.
  2. No fault injection ever closed the loop to a real process exit code.
     test_check_pipeline_output.py asserts the pure function
...
class RealEnterpriseRepoTest(unittest.TestCase):
    ...
    satisfy them. Same opt-in shape as test_partition_repo_real_world.py, so
