...
> **The grace window did not hold, and the table below records where.** This paragraph used to claim the exemption was "a bounded grace window, not a hole: the exemption shifts to whichever PR merges next, so nothing stays undocumented past one PR cycle." That reasoning only holds while the check can actually fail. `scripts/check_llms_coverage.py` has `ENFORCE = False` (set during a fast-merge burst and never flipped back), so the findings print and the build stays green — and **PRs #21–#27 merged with no `pr-N.md` at all**, seven PRs rather than one. Run `python3 scripts/check_llms_coverage.py` to see the current list. The exemption is sound in principle; what failed is that nothing enforced the window's closing. Either backfill and set `ENFORCE = True`, or drop the convention deliberately — but a CI step named as a gate that cannot fail is the worst of the three options.
>
> **Now tracked as [issue #33](https://github.com/hcook17/spring-boot-doc-agent/issues/33)**, with an owner and a definition of done, rather than living only in this paragraph. Note the real count is easy to under-read here: rows 21–23 carry the same **no `pr-N.md`** marker as 24–27, so the table looks like ~5 owed when it is **8** — 9 counting #31, which is owed but currently exempt as most-recently-merged. That is the true cost of flipping the flag.
...
3. **Never backtick-quote a git or gh command in *prose* inside the verification section — it will be executed.** `parse_commands()` collects every backtick-fenced span in that section matching `^(git|gh)\s`. It does not distinguish a command you intend to run from one you are merely naming. Writing "the git-am step misreported X" with backticks around the command name schedules that command for execution.

   This is not only a false-failure problem. **The harness will run mutating commands.** A draft of `pr-32.md` named one three times in prose; the verifier executed it against the live repo, created a stray `.git/rebase-apply`, and left the working tree mid-operation until it was aborted by hand. Name commands in prose without backticks, or describe them ("the patch-application step"). Tracked as [issue #35](https://github.com/hcook17/spring-boot-doc-agent/issues/35).
