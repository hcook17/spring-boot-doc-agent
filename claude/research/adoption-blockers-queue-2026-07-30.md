...
**Landed:** `rule_coverage_baseline.json` → `schema_version` 2 (orphan pack-less keys dropped; no invented counts for unmeasured pack ids); `check_ratchet` / `check_non_vacuity` fail-closed (missing baseline, corrupt JSON, missing/non-object `counts`, empty pack); hermetic committed-schema witness in `tests/coverage/test_rule_coverage.py`; `codeql_rule_count` derivation in `check_repo_claims.py` + CLAUDE derived block. **Standing ban:** do **not** invent client-named semgrep recall baseline (`dev-fp-ratchet-separate-from-recall`).
