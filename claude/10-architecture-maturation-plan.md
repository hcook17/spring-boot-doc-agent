...
date: 2026-07-24
supersedes: nothing; consolidates code-review_4, deep-dive_4, er-model-review_3
artifacts: scripts/local_ci.sh, .github/workflows/_python-checks.yml, .github/workflows/ci.yml, PORTING.md
see-also: claude/archunit-scanner-scoping-2026-07-23.md; "Closing the JPA/Hibernate Predicate Vocabulary: A Bounded Completeness Survey for Static Extraction.md"; "Hibernate ORM and Jakarta Persistence Claims: July 2026 Fact-Verification.md"
...
- 0.1.4 `zizmor` and `actionlint` as gates (already wired into `_python-checks.yml`). Point them at the org's five workflows too — the Sonar-token-on-the-command-line finding is precisely what zizmor exists to catch.
...
- 0.2.1 `carry_forward()` must never carry the entire closed group — iterate `reversed(closed_group[1:])` — and assert `len(carry) < len(closed_group)` afterward. Forward progress becomes unconditional.
...
- 0.2.5 Add a `max_group_tokens` warning dimension to `compute_preflight()`; emit oversize files in a dedicated array (M4). Today the preflight tool hangs on precisely the repos it exists to warn about.
...
- 0.4.2 Replace `sys.exit(1)` in `run_ast_grep()` with `AstGrepInvocationError`; catch in all three `main()`s; in `tier2_recheck_file`, catch per-file and mark `unknown_recheck_failed` rather than aborting a whole run (M2).
