...
> **Current as of 2026-07-29 (read this first).** Product SoT is **doc-engine** ([`docs/product-architecture.md`](../docs/product-architecture.md)): Stage 0 tools live under `src/doc_engine/tools/`, invoke via `doc-engine` / `python -m doc_engine.tools.*`, Claude plugin is an adapter under `adapters/claude/`. **Keep** §0 reframe and Phase 1–3 (fact store / derivation / breadth) as the locked next engineering investment (`STATUS.md`). **Do not** treat Phase 0.1 Elsevier-port deliverables (`PORTING.md`, `scripts/local_ci.sh`, `.github/workflows/_python-checks.yml`) as current work — they were never landed and are superseded by the portable-kernel CI already in `.github/workflows/ci.yml`. Path cites in the body that say `scripts/spring_signal_scan.py` (etc.) mean `src/doc_engine/tools/` today. Packaging/hygiene (A+C, shim deletion, suite layout) is paused as complete; do not mix new packaging mega-PRs with fact-store Phase 1.
