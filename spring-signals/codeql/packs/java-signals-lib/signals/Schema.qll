 ...
 *   framework       "spring" | "jpa" | "hibernate" | "openapi" | "sql" | "jakarta"
...
string symbolOf(Element e) {
  exists(RefType t | t = e and result = typeSymbol(t))
  or
  ...
  exists(Parameter p, Callable c |
    p = e and c = p.getCallable() and
    result = symbolOf(c) + "(" + p.getName() + ")"
  )
  or
  // Anything else (annotations, statements, expressions) is identified by its
  // nearest named enclosing declaration, which is what a cross-tool join can
  // actually agree on.
  not e instanceof RefType and
  not e instanceof Callable and
  not e instanceof Field and
  not e instanceof Parameter and
  result =
    min(string s |
      exists(Callable c | c = e.(Expr).getEnclosingCallable() and s = symbolOf(c))
      or
      exists(Callable c | c = e.(Stmt).getEnclosingCallable() and s = symbolOf(c))
      or
      exists(RefType t | t = e.(Annotation).getAnnotatedElement().(RefType) and s = typeSymbol(t))
    |
      s
    )
}
