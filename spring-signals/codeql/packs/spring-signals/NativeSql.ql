...
bindingset[sql]
private predicate usesJson(string sql) {
  sql.regexpMatch("(?is).*(->>|->|#>>|jsonb_|json_build_object|jsonb_array_elements).*")
}
...
where
  ...
  exists(MethodCall call, string pkg, string name |
    ...
    sql =
      normalizeWhitespace(concat(Argument arg |
          arg = call.getAnArgument() and arg.getType() instanceof TypeString
        |
          concatenatedText(arg), " " order by arg.getPosition()
        )) and
