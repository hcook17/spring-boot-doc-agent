...
# Credentials travel through the environment, not --command: -P values on the
# command line are visible in `ps` and land in the CodeQL build log inside the
# database directory. ORG_GRADLE_PROJECT_<name> is Gradle's env-to-project-
# property mapping, so build.gradle reads the same property names unchanged.
