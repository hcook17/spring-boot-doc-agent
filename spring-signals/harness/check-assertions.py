...
Reads two files from the same directory as this script:
...
def count_by_rule_id(query: str, rule_id: str) -> int:
    csv_path = OUT / f"{query}.csv"
    if not csv_path.exists():
        return -1
    with csv_path.open(newline="", encoding="utf-8") as fh:
...
def main() -> int:
    ...
    for ln in load_lines(ROOT / "expected-empty.txt"):
        parts = ln.split()
        if len(parts) != 2:
        ...
        actual = count_rows(query)
        if actual == -1:
            print(f"MISS {query} (no {OUT / query}.csv)")
            failed += 1
    ...
    for ln in load_lines(ROOT / "expected-min.txt"):
        parts = ln.split()
        if len(parts) != 3:
        ...
        expected_count = int(expected)
        actual = count_by_rule_id(query, rule_id)
        if actual == -1:
