...
Pick up `claude/steering-prompts/01-testability-research-prompt.md` (no test coverage for the LLM stages is the largest remaining gap named across all five steering prompts) — or, if enterprise rollout is the near-term goal, start `CONSTRAINTS.md`'s stated close-out order instead: license → confidentiality/secret-redaction → CI wiring → dependency pinning → audit trail / run manifest → RBAC / multi-repo.
