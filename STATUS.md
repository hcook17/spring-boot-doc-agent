...
`claude/steering-prompts/01-testability-research-prompt.md` (LLM-stage testability — `test_pipeline_stages.py` covers structural regression, but the deeper research prompt isn't closed out) is the next open steering prompt, or `CONSTRAINTS.md`'s remaining close-out order if enterprise rollout is the goal: license → confidentiality/secret-redaction → branch protection + required reviews (CI wiring itself is now done) → dependency pinning → audit trail / run manifest → RBAC / multi-repo.
