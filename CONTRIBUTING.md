...
| Complexity ≤ **5** / function | [complexipy](https://github.com/rohaquinlop/complexipy) `~=6.2.0` | 748 | **6.2.0** (2026-07-23) | 2026-08-04 | Cognitive complexity (Campbell/Sonar-inspired; not affiliated with Sonar) | **hard fail** on **all** of `src/doc_engine` + `src/stf` (whole-repo `--failed`); plus offender-count ratchet |
...
**Complexity remediation.** The complexipy gate is intentionally whole-repo at ≤5 — same rigor for legacy code as for new edits. Remaining offenders above 5 keep `quality-gates` red until remediated. `scripts/ratchets/complexipy_baseline.json` additionally fails if the offender *count* rises (ratchet downward after each batch; never raise it). Prefer named helpers and early returns over micro-fragmentation; do not weaken the ≤5 threshold.
