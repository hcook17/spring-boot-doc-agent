...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/semantic_eval_helpers.py" <artifacts_dir> --out mechanical_findings.json
```
...
**Before sampling, project this pass's own cost — do not exempt this skill from the same scalability discipline it's partly built to enforce elsewhere.** Compute, using `${CLAUDE_PLUGIN_ROOT}/scripts/partition_repo.py`'s own token estimator (the same one `capacity-preflight` imports — see that skill — not a second implementation):
